<x-traveler.my.layout :title="__('nav.my_bookings')">
        ...
        <div class="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            <a href="{{ route('my.bookings', ['locale' => app()->getLocale()]) }}"
               class="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all duration-200 cursor-pointer
                ...
                {{ __('booking.filter_all') }}
            </a>
            @php
                $statusConfig = [
                    'pending' => ['label' => __('booking.filter_pending'), 'icon' => 'M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z'],
                    'confirmed' => ['label' => __('booking.filter_confirmed'), 'icon' => 'M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                    'completed' => ['label' => __('booking.filter_completed'), 'icon' => 'M9 12.75L11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 01-1.043 3.296 3.745 3.745 0 01-3.296 1.043A3.745 3.745 0 0112 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.746 0 01-3.296-1.043 3.745 3.745 0 01-1.043-3.296A3.745 3.745 0 013 12c0-1.268.63-2.39 1.593-3.068a3.745 3.745 0 011.043-3.296 3.746 3.746 0 013.296-1.043A3.746 3.746 0 0112 3c1.268 0 2.39.63 3.068 1.593a3.746 3.746 0 013.296 1.043 3.746 3.746 0 011.043 3.296A3.745 3.745 0 0121 12z'],
                    'cancelled' => ['label' => __('booking.filter_cancelled'), 'icon' => 'M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                ];
            @endphp
    ...
    @if($bookings->isEmpty())
        <div class="bg-white rounded-2xl shadow-sm p-12 text-center">
            ...
            <h3 class="text-xl font-bold text-gray-900 mb-2">{{ __('booking.no_bookings') }}</h3>
            <p class="text-gray-500 mb-6">{{ __('booking.no_bookings_desc') }}</p>
                        ...
                        <div class="flex-1 p-5 sm:p-6">
                            ...
                            <div class="flex flex-wrap items-center gap-4 text-sm text-gray-600">
                                ...
                                <div class="flex items-center gap-1.5">
                                    ...
                                    {{ __('booking.adults_count', ['count' => $booking['adult_count']]) }}
                                    @if($booking['child_count'] > 0)
                                        , {{ __('booking.children_count', ['count' => $booking['child_count']]) }}
                                    @endif
                                </div>
                            </div>
