<x-layouts.admin>
    <x-slot name="header">지역 수정</x-slot>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/60 p-6 space-y-6">
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="code" value="{{ old('code', $region->code) }}" required
                    ...
                    <div>
                        ...
                        <select name="level" required id="level-select"
                                class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors">
                            <option value="1" {{ old('level', $region->level) == 1 ? 'selected' : '' }}>국가</option>
                            <option value="2" {{ old('level', $region->level) == 2 ? 'selected' : '' }}>시/도</option>
                            <option value="3" {{ old('level', $region->level) == 3 ? 'selected' : '' }}>시/군/구</option>
                        </select>
                    </div>
                </div>
                        ...
                        @if(isset($parents[1]))
                            <optgroup label="국가">
                                @foreach($parents[1] as $country)
                                    <option value="{{ $country->id }}" data-level="1" {{ old('parent_id', $region->parent_id) == $country->id ? 'selected' : '' }}>
                                        {{ $country->getName('ko') }}
                                    </option>
                                @endforeach
                            </optgroup>
                        ...
                        @if(isset($parents[2]))
                            <optgroup label="시/도">
                                @foreach($parents[2] as $province)
                                    <option value="{{ $province->id }}" data-level="2" {{ old('parent_id', $region->parent_id) == $province->id ? 'selected' : '' }}>
                                        {{ $province->getName('ko') }}
                                    </option>
                                @endforeach
                            </optgroup>
                ...
                @php
                    $koTranslation = $region->translations->firstWhere('locale', 'ko');
                    $enTranslation = $region->translations->firstWhere('locale', 'en');
                @endphp
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="translations[ko][short_name]" value="{{ old('translations.ko.short_name', $koTranslation?->short_name) }}"
                                       class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
                                       placeholder="예: 서울">
                            </div>
                        </div>
                        <div>
                            ...
                            <textarea name="translations[ko][description]" rows="2"
                                      class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
                                      placeholder="지역 설명 (선택)">{{ old('translations.ko.description', $koTranslation?->description) }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="translations[en][short_name]" value="{{ old('translations.en.short_name', $enTranslation?->short_name) }}"
                                       class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
                                       placeholder="e.g., Seoul">
                            </div>
                        </div>
                        <div>
                            ...
                            <textarea name="translations[en][description]" rows="2"
                                      class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
                                      placeholder="Region description (optional)">{{ old('translations.en.description', $enTranslation?->description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="border-t border-slate-200 pt-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.0000001" name="latitude" value="{{ old('latitude', $region->latitude) }}"
                                   class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
                                   placeholder="37.5665">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">경도</label>
                            <input type="number" step="0.0000001" name="longitude" value="{{ old('longitude', $region->longitude) }}"
                                   class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
                ...
                <div class="border-t border-slate-200 pt-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="sort_order" value="{{ old('sort_order', $region->sort_order) }}" min="0"
                                   class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">타임존</label>
                            <input type="text" name="timezone" value="{{ old('timezone', $region->timezone) }}"
                                   class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors">
                        </div>
                    </div>
                    <div class="flex gap-6 mt-4">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="is_featured" value="1"
                                   class="rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                                   {{ old('is_featured', $region->is_featured) ? 'checked' : '' }}>
