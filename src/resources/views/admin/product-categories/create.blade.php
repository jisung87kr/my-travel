<x-layouts.admin>
    <x-slot name="header">새 카테고리</x-slot>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/60 p-6 space-y-6">
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="level" required id="level-select"
                                class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors">
                            <option value="1" {{ old('level', 1) == 1 ? 'selected' : '' }}>대분류</option>
                            <option value="2" {{ old('level') == 2 ? 'selected' : '' }}>중분류</option>
                            <option value="3" {{ old('level') == 3 ? 'selected' : '' }}>소분류</option>
                        </select>
                    </div>
                </div>
                        ...
                        @if(isset($parents[1]))
                            <optgroup label="대분류">
                                @foreach($parents[1] as $main)
                                    <option value="{{ $main->id }}" data-level="1" {{ old('parent_id') == $main->id ? 'selected' : '' }}>
                                        {{ $main->getName('ko') }}
                                    </option>
                                @endforeach
                            </optgroup>
                        ...
                        @if(isset($parents[2]))
                            <optgroup label="중분류">
                                @foreach($parents[2] as $sub)
                                    <option value="{{ $sub->id }}" data-level="2" {{ old('parent_id') == $sub->id ? 'selected' : '' }}>
                                        {{ $sub->getName('ko') }}
                                    </option>
                                @endforeach
                            </optgroup>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="translations[ko][description]" rows="2"
                                      class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
                                      placeholder="카테고리 설명 (선택)">{{ old('translations.ko.description') }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="translations[en][description]" rows="2"
                                      class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
                                      placeholder="Category description (optional)">{{ old('translations.en.description') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="border-t border-slate-200 pt-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div id="svg-input-section" class="{{ old('icon_type') === 'image' ? 'hidden' : '' }}">
                            ...
                            <textarea name="icon_svg" rows="4" id="icon-svg-input"
                                      ...
                                      placeholder='<svg viewBox="0 0 24 24">...</svg>'>{{ old('icon_svg') }}</textarea>
                            <p class="mt-1 text-xs text-slate-500">Heroicons, Lucide 등의 SVG 아이콘 코드를 붙여넣으세요</p>
                        </div>

                        <div id="image-input-section" class="{{ old('icon_type') !== 'image' ? 'hidden' : '' }}">
