<x-layouts.admin>
    <x-slot name="header">상품 카테고리 관리</x-slot>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.product-categories.index', ['level' => 2]) }}"
                   class="px-3 py-1.5 text-sm rounded-lg {{ request('level') == 2 ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200' }} transition-colors">
                    중분류
                </a>
                <a href="{{ route('admin.product-categories.index', ['level' => 3]) }}"
    ...
    <div class="mb-6 bg-white rounded-2xl border border-slate-200/60 p-4">
        <form method="GET" action="{{ route('admin.product-categories.index') }}" class="flex flex-wrap gap-4 items-end">
            @if(request('level'))
            ...
            <div class="min-w-[200px]">
                <label class="block text-sm font-medium text-slate-700 mb-1">상위 카테고리</label>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.product-categories.index', request('level') ? ['level' => request('level')] : []) }}"
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="bg-slate-50/80 border-b border-slate-200/60">
                        ...
                        @if(request('level') >= 2)
                        <th class="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">상위 카테고리</th>
                        @endif
                        <th class="px-6 py-4 text-center text-xs font-semibold text-slate-600 uppercase tracking-wider">상품 수</th>
                        <th class="px-6 py-4 text-center text-xs font-semibold text-slate-600 uppercase tracking-wider">하위</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($categories as $category)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="font-medium text-slate-800">{{ $category->getName('ko') }}</div>
                                @if($category->getTranslation('en')?->name)
                                    <div class="text-sm text-slate-500">{{ $category->getName('en') }}</div>
                                @endif
                            </td>
                            @if(request('level') >= 2)
                            <td class="px-6 py-4 text-sm text-slate-600">
                                {{ $category->parent?->getName('ko') ?? '-' }}
                            </td>
                            @endif
                            <td class="px-6 py-4 text-center text-sm text-slate-600">{{ $category->products_count }}</td>
                            <td class="px-6 py-4 text-center text-sm text-slate-600">
                                @if($category->children_count > 0)
                                    <a href="{{ route('admin.product-categories.index', ['level' => $category->level + 1, 'parent_id' => $category->id]) }}"
                            ...
                            <td class="px-6 py-4 text-center">
                                <form method="POST" action="{{ route('admin.product-categories.toggle-active', $category) }}" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-lg transition-colors {{ $category->is_active ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                            ...
                            <td class="px-6 py-4 text-center">
                                <form method="POST" action="{{ route('admin.product-categories.toggle-featured', $category) }}" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-lg transition-colors {{ $category->is_featured ? 'bg-amber-100 text-amber-700 hover:bg-amber-200' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                                        <svg class="w-3.5 h-3.5" fill="{{ $category->is_featured ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    @if($category->products_count === 0 && $category->children_count === 0)
                                        <form method="POST" action="{{ route('admin.product-categories.destroy', $category) }}" class="inline"
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ request('level') >= 2 ? '9' : '8' }}" class="px-6 py-16 text-center">
