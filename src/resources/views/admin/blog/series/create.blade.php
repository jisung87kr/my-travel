<x-layouts.admin>
    <x-slot name="header">새 시리즈</x-slot>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('admin.blog.series.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="bg-white rounded-2xl border border-slate-200/60 p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                              class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
                              placeholder="시리즈 설명">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div id="thumbnailPreview" class="hidden aspect-video rounded-xl bg-slate-100 overflow-hidden mb-2">
