...
<div {{ $attributes->merge(['class' => $compact ? 'w-64 bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-50' : 'w-full md:w-[350px] bg-white rounded-3xl shadow-[0_8px_28px_rgba(0,0,0,0.28)] border border-gray-100 py-6 px-6 z-50 overflow-hidden']) }}>
    ...
    <div class="{{ $compact ? 'py-1' : '' }}">
        ...
        <div class="{{ $compact ? '' : 'space-y-1' }}">
            ...
            <button type="button"
                    @click="destination = '{{ is_array($region) ? $region['name'] : $region->getName(app()->getLocale()) }}'; showDestination = false; showDate = true"
                ...
                <span class="{{ $compact ? '' : 'font-semibold text-gray-700 group-hover:text-gray-900' }}">{{ is_array($region) ? $region['name'] : $region->getName(app()->getLocale()) }}</span>
            </button>
            @endforeach
        </div>
    </div>
